...
import ani.saiyako.currActivity
import ani.saiyako.currContext
import ani.saiyako.download.anime.OfflineAnimeModel
import ani.saiyako.download.manga.OfflineMangaModel
    ...
    companion object {
        ...
        fun loadMediaCompat(downloadedType: DownloadedType): Media? {
            ...
            val directory = File(
                ...
                "Saiyako/$type/${downloadedType.titleName}"
            )
        ...
        fun removeDownloadCompat(context: Context, downloadedType: DownloadedType, toast: Boolean) {
            val directory = if (downloadedType.type == MediaType.MANGA) {
                File(
                    context.getExternalFilesDir(Environment.DIRECTORY_DOWNLOADS),
                    "Saiyako/Manga/${downloadedType.titleName}/${downloadedType.chapterName}"
                )
            } else if (downloadedType.type == MediaType.ANIME) {
                File(
                    context.getExternalFilesDir(Environment.DIRECTORY_DOWNLOADS),
                    "Saiyako/Anime/${downloadedType.titleName}/${downloadedType.chapterName}"
                )
            } else {
                File(
                    context.getExternalFilesDir(Environment.DIRECTORY_DOWNLOADS),
                    "Saiyako/Novel/${downloadedType.titleName}/${downloadedType.chapterName}"
                )
            }
        ...
        private val animeLocation = "Saiyako/Anime"
    }
}
